...
client.on("message", function (message) {
    ...
    if (message.content === prefix + "summon" ) {
        if(bdd[`${idAuthorId}`]["Primo"] >= 160){
            ...
            //4 stars⭐⭐⭐⭐
                ...
                rarete = Math.floor((Math.random() * 63) + 0)
                //rarete = 10
    ...
    if(message.content === prefix + "inventaire"){
        ...
        for(item in bdd[idAuthorId]["personnage"]){  
            inventaireP.push(item)
        }
        for(item in bdd[idAuthorId]["arme"]){  
            inventaireA.push(item)
        }

        var invEmbed = new Discord.MessageEmbed()
            .setColor(`#581845`)
            .setTitle(`Inventaire de ${idAuthor}`)
            .setThumbnail(`${avatar}`)
    ...
    //$gift
    if(message.content === prefix + "gift"){
        var gift = 160;
        if(idAuthorId === "332503820271616000"){
            idUser.forEach(element => {
            ...
            message.channel.send("Tu n'as pas les droits de faire des cadeaux !")
        }
    }
